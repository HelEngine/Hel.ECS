...
namespace Hel.ECS.Entities.Matcher.Groups
{
        ...
        where TComponent3 : struct, IComponent
    {
        ...
        private delegate (TComponent1?, TComponent2?, TComponent3?) SystemMethods(
            float deltaTime, int id,
            ...
            for (_entityCounter = 0; _entityCounter < Entities.Size; _entityCounter++)
            {
                if (Entities[_entityCounter])
                {
                    var entityData = RunSystemEvents?.Invoke(
                        deltaTime,
                        _entityCounter,
                        (TComponent1) Component1[_entityCounter],
                        (TComponent2) Component2[_entityCounter],
                        (TComponent3) Component3[_entityCounter]);
        ...
        private void UpdateValues((TComponent1?, TComponent2?, TComponent3?)? values)
        {
            ...
            if (values.Value.Item1.HasValue) 
                Component1.UpdateComponent(_entityCounter, values.Value.Item1.Value);
            
            if (values.Value.Item2.HasValue) 
                Component2.UpdateComponent(_entityCounter, values.Value.Item2.Value);
            
            if (values.Value.Item3.HasValue) 
                Component3.UpdateComponent(_entityCounter, values.Value.Item3.Value);
        }
